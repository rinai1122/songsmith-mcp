...
- **`render/vocal/`** — pluggable singing-voice backends for the `melody`/`vocal` role, selected via `SONGSMITH_VOCAL_BACKEND`: `formant` (default, vowel-shaped additive synth driven by `Note.lyric` — strict upgrade over the saw lead, always available), `nnsvs` (real neural SVS via `pip install nnsvs pysinsy` + `SONGSMITH_VOCAL_BANK=/path/to/release-dir`), `external` (subprocess plug-in via `SONGSMITH_VOCAL_RENDER_CMD='renderer --in {input} --out {output}'` — the JSON↔WAV contract is documented in `vocal/external.py`; reference NNSVS implementation lives at `examples/vocal_renderer_nnsvs.py`), `saw` (legacy parity). `auto` (default) tries nnsvs → external → formant. `pipeline.render_song` catches any vocal-backend exception and falls back to formant so a misconfigured engine never breaks the mix.
